...
foreach ($produits as $row) {
    if ($currentProduit !== $row['libelle']) {
        ...
        $currentProduit = $row['libelle'];

        // Ouvrir une nouvelle div pour la catégorie
        echo "<div class='categorie'>";
    }

    echo "<a href='/info.php?id=" . $row['id'] . "&amp;titre=" . $row['titre'] . "&amp;direction=produits'><div class='card'>";
    ...
    if (isset($_SESSION['admin_logged_in']) && $_SESSION['admin_logged_in'] === true) {
        ...
        if ($categoryCounts[$row['libelle']] === 1) {
            ...
            echo "<div><a href='#' class='change-order-link' data-direction='droite' data-service-id='$produitId'>&#9654;</a></div>";
        }
        // Vérifier si c'est le dernier service de sa catégorie
            ...
            echo "<div><a href='#' class='change-order-link' data-direction='gauche' data-produit-id='$produitId'>&#9664;</a></div>";
        }
        // Pour les services autres que le premier et le dernier de leur catégorie
        else {
            // Afficher les deux boutons
            echo "<div><a href='#' class='change-order-link' data-direction='gauche' data-produit-id='$produitId'>&#9664;</a>";
            echo "<a href='#' class='change-order-link' data-direction='droite' data-produit-id='$produitId'>&#9654;</a></div>";
        }
