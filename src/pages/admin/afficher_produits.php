...
foreach ($produits as $key => $row) {
    ...
    echo "<a href='/info.php?id=" . $row['id'] . "&amp;titre=" . $row['titre'] . "&amp;direction=produits'><div>";
    ...
    if (isset($_SESSION['admin_logged_in']) && $_SESSION['admin_logged_in'] === true) {
        ...
        // Vérifier si c'est le premier produit de sa catégorie
        if ($categoryCounts[$row['libelle']] === 1) {
            // Afficher uniquement le bouton de droite
            echo "<div><a href='#' class='change-order-link' data-direction='droite' data-product-id='$productId'>&#9654;</a></div>";
        }
        // Vérifier si c'est le dernier produit de sa catégorie
        else if ($categoryCounts[$row['libelle']] === count(array_filter($produits, function($produit) use ($row) {
                return $produit['libelle'] === $row['libelle'];
            }))) {
            // Afficher uniquement le bouton de gauche
            echo "<div><a href='#' class='change-order-link' data-direction='gauche' data-product-id='$productId'>&#9664;</a></div>";
        }
        // Pour les produits autres que le premier et le dernier de leur catégorie
        else {
            // Afficher les deux boutons
            echo "<div><a href='#' class='change-order-link' data-direction='gauche' data-product-id='$productId'>&#9664;</a>";
            echo "<a href='#' class='change-order-link' data-direction='droite' data-product-id='$productId'>&#9654;</a></div>";
        }

        // Boutons pour supprimer et modifier le produit
        echo "<div>";
        echo "<a onclick='return confirmDelete();' class='admin_action_special' data-id='" . $row['id'] . "'><svg xmlns='http://www.w3.org/2000/svg' height='16' width='18' viewBox='0 0 576 512'><path d='M576 128c0-35.3-28.7-64-64-64H205.3c-17 0-33.3 6.7-45.3 18.7L9.4 233.4c-6 6-9.4 14.1-9.4 22.6s3.4 16.6 9.4 22.6L160 429.3c12 12 28.3 18.7 45.3 18.7H512c35.3 0 64-28.7 64-64V128zM271 175c9.4-9.4 24.6-9.4 33.9 0l47 47 47-47c9.4-9.4 24.6-9.4 33.9 0s9.4 24.6 0 33.9l-47 47 47 47c9.4 9.4 9.4 24.6 0 33.9s-24.6 9.4-33.9 0l-47-47-47 47c-9.4 9.4-24.6 9.4-33.9 0s-9.4-24.6 0-33.9l47-47-47-47c-9.4-9.4-9.4-24.6 0-33.9z'/></svg></a>";
